...
class GenFiles:
    ...
    steering_file = 'steering_{}_{:07}.npy'
    diff_file = 'diff_{}_{:07}.npy'
    steering_diff_file = 'steering_diff_{}_{:07}.npy'


class Generator:
    def __init__(self, config, memory_tuple=None, base_path=None, batch_size=32, shuffle=True):
    ...
    def generate(self, data='train', column_mode='all'):
        if data == 'train':
        ...
        while True:
            ...
            for i in range(batch_count):
                ...
                x_frame, x_numeric, y = self.__load_batch(batch_indexes, column_mode)

                yield (x_frame, x_numeric), y

    def __load_batch(self, batch_indexes, column_mode):
        frames = []
        numerics = []
        diffs = []
        ...
        for i in batch_indexes:
            ...
            if column_mode == 'steering':
                numeric = np.load(self.path + GenFiles.steering_file.format(self.memory_string, i), allow_pickle=True)
                diff = np.load(self.path + GenFiles.steering_diff_file.format(self.memory_string, i), allow_pickle=True)

                numeric = self.__memory.columns_from_memorized(numeric, (1, 2))
                diff = diff[:, 1]
            elif column_mode == 'all':
